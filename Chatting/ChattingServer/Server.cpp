...
bool Dup_Check(string id);
void Select(string table);
int RowsCount_user_info();
//------------------------
...
//user_info id 중복 체크, 반환값이 true면 중복
bool Dup_Check(string id) {
    sql::ResultSet* result;

    string str = "select * from user_info where id = ";
    str += id + ";";
...
void add_client() {
    ...
    int user_info_counts = RowsCount_user_info(); //user_info DB에 등록되어 있는 유저들 수
    while(true) {
